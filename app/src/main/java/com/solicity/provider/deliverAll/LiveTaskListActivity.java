            ...
            if (responseStringObject != null && !responseStringObject.equals("")) {
                ...
                if (generalFunc.checkDataAvail(Utils.action_str, responseStringObject)) {
                    ...
                    for (int i = 0; i < msgArray_obj.length(); i++) {
                     ...
                     //   model2.seteBuyAnyService(eBuyAnyService);
                      //  model2.setGenieOrderType(GenieOrderType);
...
//                        model2.setRestaurantImage(generalFunc.getJsonValueStr("vRestuarantImage", msg_obj));
                        ...
                        model.setRestaurantAddress(Utils.checkText(restAddress) ? WordUtils.capitalize(restAddress) : restAddress);
...
//                        model2.setLBL_NEXT_TASK_TXT(generalFunc.retrieveLangLBl("Next Task", "LBL_NEXT_TASK_TXT"));
                        ...
                        //if(model.getIsForPickup().equals("Yes")) {
                           // list.add(model);
                       // }


                        //if (generalFunc.getJsonValueStr("eBuyAnyService", msg_obj).equalsIgnoreCase("No")) {
                        if (generalFunc.getJsonValueStr("PickedFromRes", msg_obj).equalsIgnoreCase("No") || (generalFunc.getJsonValueStr("PickedFromRes", msg_obj).equalsIgnoreCase("Yes") && generalFunc.getJsonValueStr("isPhotoUploaded", msg_obj).equalsIgnoreCase("No") && eBuyAnyService.equalsIgnoreCase("No") )) {
                         ...
                         //   model2.setIsRestaurant("Yes");
                           // model2.setIsForPickup(generalFunc.getJsonValue("isForPickup",msg_obj).toString());
                          //  model2.setIsForDropoff(generalFunc.getJsonValue("isForDropoff",msg_obj).toString());
//                            if(model2.getIsForDropoff().equals("Yes")) {
//                                list.add(model2);
//                            }
                        }
                        else
    ...
    public void sinchCall(boolean isStore,int pos) {
        ...
        if (isStore) {
            ...
            if (!list.get(pos).getPPicName().equals("")) {
                vImage = list.get(pos).getPPicName();
            }
            else

            vName = list.get(pos).getUserName();
        }
        ...
        if (generalFunc.isCallPermissionGranted(false) == false) {
            ...
            if (new AppFunctions(getActContext()).checkSinchInstance(getSinchServiceInterface())) {
                ...
                if (isStore) {
                    ...
                    Call call = getSinchServiceInterface().callUser(Utils.CALLTOPASSENGER + "_" +list.get(pos).getiTripId(), hashMap);
                    String callId = call.getCallId();
                    Intent callScreen = new Intent(getActContext(), CallScreenActivity.class);
                    callScreen.putExtra(SinchService.CALL_ID, callId);
                    callScreen.putExtra("vImage", vImage);
        ...
        if (pickedFromRes.equalsIgnoreCase("Call")) {
            ...
            if (list.get(position).getPickedFromRes().equalsIgnoreCase("No")) {
                ...
                if (generalFunc.getJsonValueStr("RIDE_DRIVER_CALLING_METHOD", userProfileJsonObj).equals("Voip")) {
                    sinchCall(true,position);
                } else {
                    getMaskNumber(list.get(position).getRestaurantNumber(),position);
                }
            } else {
                // call(list.get(position).getUserNumber());

                if (generalFunc.getJsonValueStr("RIDE_DRIVER_CALLING_METHOD", userProfileJsonObj).equals("Voip")) {
                    sinchCall(false,position);
                } else {
                    getMaskNumber(list.get(position).getUserNumber(),position);
                }
            }
    ...
    public void getMaskNumber(String number,int pos) {
        if (generalFunc.getJsonValueStr("CALLMASKING_ENABLED", userProfileJsonObj).equalsIgnoreCase("Yes")) {
            ...
            parameters.put("iOrderId", iOrderId);
            parameters.put("iTripid",list.get(pos).getiTripId());
