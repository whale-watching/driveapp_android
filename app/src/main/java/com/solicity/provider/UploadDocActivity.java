...
public class UploadDocActivity extends AppCompatActivity {
    ...
    public void setLabels() {
        ...
        if (getIntent().getStringExtra("ex_status").equals("yes")) {
            expBoxTXT.setText(generalFunc.getDateFormatedType(getIntent().getStringExtra("ex_date"),"yyyy-MM-dd",CommonUtilities.WithoutDayFormat));
    ...
    public void checkData() {
        ...
        paramsList.add(Utils.generateImageParams("ex_date", getIntent().getStringExtra("ex_status").equals("yes") ? generalFunc.getDateFormatedType(Utils.getText(expBoxTXT),CommonUtilities.WithoutDayFormat,"yyyy-MM-dd",Locale.US) : ""));
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == OpenSourceSelectionDialog.CAMERA_CAPTURE_IMAGE_REQUEST_CODE && resultCode == RESULT_OK) {
            ...
            if (Utils.isValidImageResolution(selPath) && isStoragePermissionAvail) {
    ...
    private void selectImage() {
        if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            ...
            if (generalFunc.isAllPermissionGranted(true,requestPermissions)) {
                ...
                generalFunc.showMessage(generalFunc.getCurrentView((Activity) getActContext()), "Allow this app to use camera.");
            }
        } else {
